...
export default function WorkOrderChangeStatusPopover(context) {
    ...
    let roleCheck = function() {
        ...
        //B.O.A for D070
        let siCloseCheck = function () {
            /*Check SI Order whether it is Closed 
            if SI Order - IF SI is closed - Allow to Complete the Order
            if Non SI Order - Allow to Complete as SAP flows
            */
    
            var orderNumber = context.binding.OrderId;
            var workOrderQueryOptions = "$filter=OrderNumber eq '" + orderNumber + "'";
            //Check SI 
            return context.read('/SmartInspections/Services/SAM.service', 'WorkOrderHeaderSet', [], workOrderQueryOptions).then(
                function (results) {
                    /*  Determine if it  is allowed to complete
                    Allow COMPLETE - 1. If it is NON SI , 2. if SI & it is closed
                    */
                    var binding = {};
                    var bAllowComplete = true;
                    if (results && results.length > 0) {
                        ...
                        if (binding && binding.CloseOrder === 'X') {
                            //Allow to COMPLETE if the SI is Closed
                            bAllowComplete = true;
                        } else {
                            bAllowComplete = false;
                        }
                    }
                    return bAllowComplete;
                });
    ...
    return Promise.all([libWOMobile.isAnyWorkOrderStarted(context), roleCheck(), DisconnectAllowComplete(context), /*D070*/ siCloseCheck() ]).then(checks => {
        ...
        let bAllowComplete = checks[3]; //++ for D070
                ...
                return context.read('/SAPAssetManager/Services/AssetManager.service', `${context.binding['@odata.readLink']}/OrderMobileStatus_Nav`, [], '$expand=OverallStatusCfg_Nav').then(rollback => {
                    ...
                    return context.read('/SAPAssetManager/Services/AssetManager.service', entitySet, [], queryOptions).then(codes => {
                                ...
                                if (supervisorRole === 'S' || (supervisorRole === 'T' && !review) && checks[2]) { //Allow complete if supervisor or if technician and WO does not require review, and all meter disconnections are done
                                    ...
                                    if (bAllowComplete) {
                                      popoverItems.push({
                                        ...
                                        //Show SI non Complete Message while CLosing - OnOk naviagte to SI
                                        popoverItems.push({
                                            'Title': transitionText, 'OnPress': {
                                                'Name': '/SAPAssetManager/Actions/Common/GenericWarningDialog.action',
                                                'Properties': {
                                                    ...
                                                    'OKCaption': context.localizeText('ok'),
                                                    'CancelCaption': context.localizeText('cancel'),
                                                    'OnOK': "/SmartInspections/Actions/CloseWorkOrderToComplete.action",
                                                },
                                            }
                        ...
                        return Promise.all([isAssignEnableWorkOrder(context), isUnAssignOrReAssignEnableWorkOrder(context)]).then(assignResults => {
                            ...
                            if (popoverItems.length > 1 || (supervisorRole === 'S' && mobileStatus === REJECTED)) {
