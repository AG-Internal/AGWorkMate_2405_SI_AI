function ExecuteUpdateEntity(clientAPI, binding) {
	clientAPI.getPageProxy().setActionBinding(binding);
	return clientAPI.executeAction('/SmartInspections/Actions/TechnicalObjectInspTypeSave_UpdateEntity.action');
}

function Wait() {
	return new Promise(r => setTimeout(r, 2000))
}

export default function InspectionTypeList_OnPress(clientAPI) {
	var binding = clientAPI.getPageProxy().getActionBinding();
	var inspectionType = binding.InspectionType;
	var inspectionTypeDesc = binding.InspectionTypeDesc;
	let listPageClientData = '';
	var latestPromise = Promise.resolve();

	try {
		listPageClientData = clientAPI.evaluateTargetPath("#Page:TechnicalObjectDetailsList/#ClientData");
		var binding = listPageClientData.NavigatedTechObject;
	} catch (err) {}

	var isoDateTime = new Date().toISOString();
	var dateTime = isoDateTime.split(".");
	var time = dateTime[0].split("T");

	latestPromise = latestPromise.then(() => {
		binding.InspectionType = inspectionType;
		binding.InspectionTypeDesc = inspectionTypeDesc;
		binding.ChangedOn = new Date().toISOString();
		binding.ChangedTime = time[1];
		binding.Status = 'T';
		return ExecuteUpdateEntity(clientAPI, binding);
	}).then(Wait);

	return latestPromise.then(function () {
		return clientAPI.executeAction('/SmartInspections/Actions/InspectionTypeDetailsList_Close.action');
	});
}