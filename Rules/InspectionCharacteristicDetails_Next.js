export default function InspectionCharacteristicDetails_Next(clientAPI) {
	...
	//alert(Object.entries(nextBinding));
	//clientAPI.getPageProxy().redraw();
	pageProxy.setActionBinding(nextBinding);

	if (micType === nextMicType) {
		clientAPI.executeAction('/SmartInspections/Actions/NavToQuantitative.action');
		clientAPI.getPageProxy().redraw();
		//return;
	}
